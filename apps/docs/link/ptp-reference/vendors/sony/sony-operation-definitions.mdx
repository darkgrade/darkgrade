...
## SDIO_GetControlDeviceDesc (`0x9206`)

Reads the descriptor and accepted values for a Sony momentary control when the connected body publishes descriptor data. Some bodies return PTP `OK` with an empty payload; callers must treat that as unsupported rather than inferring writable values.

| Parameter        | Type     | Required | Description                   |
| ---------------- | -------- | -------- | ----------------------------- |
| `sdiControlCode` | `uint32` | Yes      | Sony control code to describe |
